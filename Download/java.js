...
function makeRequest(inputUrl, retries = 4) {
    const requestUrl = `//vkrdownloader.xyz/server?api_key=vkrdownloader&vkr=${encodeURIComponent(inputUrl)}`;
...
function handleSuccessResponse(data, inputUrl) {
    ...
    ? `//i.ytimg.com/vi/${videoId}/hqdefault.jpg`
    : videoData.thumbnail;
        ...
        const videoHtml = `
            <video style='background: black url(${thumbnailUrl}) center center/cover no-repeat; width:100%; height:500px; border-radius:20px;' 
                ...
                <source src='//invidious.nerdvpn.de/latest_version?id=${videoId}&itag=18&local=true' type='video/mp4'>
                <source src='//invidious.jing.rocks/latest_version?id=${videoId}&itag=18&local=true' type='video/mp4'>
                <source src='//vkrcors.vercel.app/proxy?url=//eu-proxy.poketube.fun/latest_version?id=${videoId}&itag=18&local=true' type='video/mp4'>
        ...
        displayError("Issue: Unable to retrieve the download link. Please check the URL and contact us on Social Media @Only_botz.");
        document.getElementById("loading").style.display = "none";
    }
}
/* NEW */
...
function generateDownloadButtons(videoData, inputUrl) {
    ...
    if (videoData.data) {
        ...
        if (videoId) {
            downloadContainer.innerHTML += `
                <a href='//inv.nadeko.net/latest_version?id=${videoId}&itag=18&local=true' target='_blank' rel='noopener noreferrer'>
      ...
      <iframe style="border: 0; outline: none; width: 100%; max-height: 45px; height: 45px !important; margin-top: 10px; overflow: hidden;"   sandbox="allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox allow-downloads allow-downloads-without-user-activation"  scrolling="no"
       src="//vkrdownloader.xyz/server/dlbtn.php?q=${encodeURIComponent(quality)}&vkr=${encodeURIComponent(videoSource)}">
       </iframe>`;
            });
        }
    ...
    if (downloadContainer.innerHTML.trim() === "") {
        displayError("Server Down due to Too Many Requests. Please contact us on Social Media @Only_Botz.");
        document.getElementById("container").style.display = "none";
        // Redirecting the user to an alternative download page
        window.location.href = `//vkrdownloader.xyz/download.php?vkr=${encodeURIComponent(inputUrl)}`;
    }
}
